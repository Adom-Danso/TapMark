...
export async function getCartInvoice(
    cartId: string,
    longitude: number,
    latitude: number,
): Promise<SuccessResponse<CartInvoice>> {
    try {
        const response = await axiosInstance.get(`/orders/invoice/${cartId}`, {
            params: {
                lng: longitude,
                lat: latitude,
            }
        });
        return response.data as SuccessResponse<CartInvoice>;
    } catch (error) {
