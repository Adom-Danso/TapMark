...
import { Bank } from "@/schemas/payments";


export async function getPaystackBanks(

): Promise<SuccessResponse<Bank[]>> {
    try {
        const response = await axiosInstance.get('/payments/paystack/banks');
        return response.data as SuccessResponse<Bank[]>;
